...
| 0.2 | Backend updated to Supabase + Vercel. Cron ping added. Visual design tokens section added. |
| 0.3 | Settlo → Yaari Khaatha. TypeScript, TanStack Query v5, Tailwind v4, Mocha Premium palette (light+dark), revised typography, 4-tab + FAB nav, email/password + Google OAuth, linked_user_id on persons, equal splits only, read-only offline. |
...
| Language | TypeScript (relaxed) | Type safety without friction |
| Styling | Tailwind CSS v4 | `@theme` for design tokens, CSS-native config |
...
| QR Code | qrcode.react | Client-side UPI QR generation |

### Backend
| Layer | Choice | Reason |
|-------|--------|--------|
| Backend | Supabase | PostgreSQL, RLS, Auth built-in |
| Database | PostgreSQL (via Supabase) | Relational — correct model for ledger data |
| Auth | Email/password + Google OAuth | Email OTP for verification (not SMS) |
| Hosting — Frontend | Vercel | Free, instant deploys |
| Hosting — Backend | Supabase free tier | With cron-job.org ping for pause prevention |
...
Supabase pauses projects after 7 days of inactivity on the free tier. Fix:
1. Create a free account at cron-job.org
2. Set up a cron job to ping your Supabase REST URL every 5 days
3. Use: `GET https://[project].supabase.co/rest/v1/persons?select=id&limit=1`
...
### persons
People the user transacts with. Do NOT need to be app users.
```sql
...
### groups
Named group for recurring expenses.
```sql
create table groups (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references auth.users on delete cascade not null,
...
### transactions
A single financial event. Amounts stored as integers (paise).
```sql
...
### transaction_persons
Junction table — which persons are involved and what they owe.
```sql
...
### settlements
A repayment event. Amounts in paise.
```sql
...
### share_tokens
Read-only shareable summary links. No expiry — always live data.
```sql
create table share_tokens (
  ...
  expires_at timestamptz, -- nullable, no expiry for V1
  created_at timestamptz default now()
);
...
**Critical rule: all amounts stored and computed as integers (paise). No floats.**
...
export function getNetBalance(
  ...
  const totalSettled = settlements.reduce(
    (acc, s) => acc + s.amount_paise, 0
  )

  return txBalance - totalSettled
}
...
Both QR code and deep-link button shown on every shareable summary page.
...
Recipient opens [url]/s/[token] (no login)
  → Public Supabase query: select share_tokens where token = [token]
  → Validate: token exists (no expiry check in V1)
  → Fetch: LIVE ledger data — owner name, person name (always real name, never label), transactions, settlements
...
**Read-only offline.** TanStack Query provides caching out of the box.

| Scenario | Behaviour |
|----------|-----------|
| No network, viewing ledger | Cached data shown with "Offline" toast |
| No network, adding transaction | "You're offline — come back to log" toast |
| No network, shareable summary | "Requires connection" message |
| Network restored | TanStack Query auto-refetches stale data |

No offline queue, no localStorage sync, no conflict resolution in V1.
...
| Supabase | PostgreSQL + Auth + RLS | Free (with cron ping) |
| cron-job.org | Pause prevention ping | Free |
...
3. Configure Supabase Auth (email/password + Google OAuth)
4. Push frontend to Vercel → set env vars
5. Configure PWA icons and manifest
6. Test shareable summary flow end-to-end

---

## 13. Testing Priorities

1. **Balance calculation unit tests** — every edge case: partial settlements, group splits, multiple settlements, zero balances
2. **Split rounding** — ₹100 among 3 = [3334, 3333, 3333], never [33.33, 33.33, 33.34]
3. **UPI deep-link format** — validate for PhonePe, GPay, Paytm
4. **Share token** — valid token returns correct data, unauthenticated access only reaches public fields
5. **Offline behaviour** — cached data displays, new transaction shows offline toast
